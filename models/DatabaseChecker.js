...
class DatabaseChecker {
  ...
  async ensureDatabase(dbName) {
    ...
    try {
      ...
      console.error(
        `❌ Error ensuring database '${dbName}':`,
        error.message
      );
      return false;
    } finally {
  ...
  async testConnection(dbName) {
    ...
    try {
      ...
      console.log(
        `✅ Connection to '${dbName}' successful`
      );
      return true;
    } catch (error) {
      console.error(
  ...
  async validateBeforePool(dbName) {
    console.log(
      `🔍 Validating database setup for '${dbName}'...`
    );
    ...
    if (!dbReady) {
      throw new Error(
        `Failed to ensure database '${dbName}' exists`
      );
    }
    ...
    console.log(
      `✅ Database '${dbName}' is ready for pool creation`
    );
    return true;
  }
}
